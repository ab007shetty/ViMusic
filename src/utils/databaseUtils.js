...
const triggerDatabaseDownloadOnServer = async (userEmail) => {
  try {
   ...
   // const response = await axios.get(`http://localhost:5000/api/download-database/${userEmail}`);
	const response = await axios.get(`https://vimusic.up.railway.app/api/download-database/${userEmail}`);
